...
pub struct Settings {
  ...
  pub tracks_table_col_separators: bool,
  pub tracks_table_row_separators: bool,
...
impl Settings {
  pub fn load() -> Result<Self> {
    ...
    let res = settings::table
      .select(Self::as_select()) // needed to decouple schema column order from struct field order
